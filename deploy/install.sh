...
# OC Router Installation Script (Chinese/English bilingual)
...
INSTALL_DIR="/opt/oc-router"
SERVICE_NAME="oc-router"
SERVICE_USER="oc-router"
CONFIG_DIR="/etc/oc-router"
...
# ============================================================
# Language strings (Chinese and English)
# ============================================================
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="OC Router 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 OC Router 服务："
    ...
    ["not_installed"]="OC Router 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 OC Router。"
    ...
    ["uninstall_complete"]="OC Router 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 OC Router"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 OC Router"
    ...
    ["server_config_desc"]="配置 OC Router 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="OC Router Installation Script"
    ...
    # Completion
    ["install_complete"]="OC Router installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start OC Router service:"
    ...
    ["not_installed"]="OC Router is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove OC Router from your system."
    ...
    ["uninstall_complete"]="OC Router has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install OC Router"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove OC Router"
    ...
    ["server_config_desc"]="Configure OC Router server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/oc-router" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/oc-router" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="oc-router_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/oc-router" "$INSTALL_DIR/oc-router"
    chmod +x "$INSTALL_DIR/oc-router"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/oc-router"
}
...
install_service() {
    ...
    cat > /etc/systemd/system/oc-router.service << EOF
[Unit]
Description=OC Router - AI API Gateway Platform
...
User=oc-router
Group=oc-router
WorkingDirectory=/opt/oc-router
ExecStart=/opt/oc-router/oc-router
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=oc-router
...
start_service() {
    ...
    if systemctl start oc-router; then
        print_success "$(msg 'service_started')"
        return 0
    else
        print_error "$(msg 'service_start_failed')"
        print_info "sudo journalctl -u oc-router -n 50"
        return 1
    fi
}
...
enable_autostart() {
    ...
    if systemctl enable oc-router 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status oc-router"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u oc-router -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart oc-router"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop oc-router"
    echo ""
    echo "=============================================="
}
...
# Upgrade function
upgrade() {
    # Check if OC Router is installed
    if [ ! -f "$INSTALL_DIR/oc-router" ]; then
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/oc-router" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet oc-router; then
        print_info "$(msg 'stopping_service')"
        systemctl stop oc-router
    fi

    # Backup current binary
    cp "$INSTALL_DIR/oc-router" "$INSTALL_DIR/oc-router.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/oc-router.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/oc-router"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start oc-router

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: OC Router must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/oc-router" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="oc-router.backup.${current_version}"
        else
            backup_name="oc-router.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/oc-router" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop oc-router 2>/dev/null || true
    systemctl disable oc-router 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/oc-router.service
    systemctl daemon-reload
