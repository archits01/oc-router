...
func (s *DashboardAggregationService) Start() {
	...
	logger.LegacyPrintf("service.dashboard_aggregation", "[DashboardAggregation] 聚合作业started (interval=%v, lookback=%ds)", interval, s.cfg.LookbackSeconds)
...
func (s *DashboardAggregationService) TriggerBackfill(start, end time.Time) error {
	if s == nil || s.repo == nil {
		return errors.New("聚合服务未initialization")
	}
	if !s.cfg.BackfillEnabled {
	...
	go func() {
		...
		if err := s.backfillRange(ctx, start, end); err != nil {
			logger.LegacyPrintf("service.dashboard_aggregation", "[DashboardAggregation] 回填failed: %v", err)
		}
	}()
	return nil
}
	...
	go func() {
		...
		for i := 0; i < maxRetries; i++ {
			...
			if !errors.Is(err, errDashboardAggregationRunning) {
				logger.LegacyPrintf("service.dashboard_aggregation", "[DashboardAggregation] 重新计算failed: %v", err)
				return
			}
...
func (s *DashboardAggregationService) recomputeRecentDays() {
	...
	if err := s.backfillRange(ctx, start, now); err != nil {
		logger.LegacyPrintf("service.dashboard_aggregation", "[DashboardAggregation] started重算failed: %v", err)
		return
	}
}
...
func (s *DashboardAggregationService) runScheduledAggregation() {
	...
	if err != nil {
		logger.LegacyPrintf("service.dashboard_aggregation", "[DashboardAggregation] 读取水位failed: %v", err)
	...
	if err := s.aggregateRange(ctx, start, now); err != nil {
		logger.LegacyPrintf("service.dashboard_aggregation", "[DashboardAggregation] 聚合failed: %v", err)
		return
	}

	updateErr := s.repo.UpdateAggregationWatermark(ctx, now)
	if updateErr != nil {
		logger.LegacyPrintf("service.dashboard_aggregation", "[DashboardAggregation] update水位failed: %v", updateErr)
	}
...
func (s *DashboardAggregationService) aggregateRange(ctx context.Context, start, end time.Time) error {
	...
	if err := s.repo.EnsureUsageLogsPartitions(ctx, end); err != nil {
		logger.LegacyPrintf("service.dashboard_aggregation", "[DashboardAggregation] 分区检查failed: %v", err)
	}
...
func (s *DashboardAggregationService) maybeCleanupRetention(ctx context.Context, now time.Time) {
	...
	if aggErr != nil {
		logger.LegacyPrintf("service.dashboard_aggregation", "[DashboardAggregation] 聚合保留cleanupfailed: %v", aggErr)
	}
	usageErr := s.repo.CleanupUsageLogs(ctx, usageCutoff)
	if usageErr != nil {
		logger.LegacyPrintf("service.dashboard_aggregation", "[DashboardAggregation] usage_logs 保留cleanupfailed: %v", usageErr)
	}
	dedupErr := s.repo.CleanupUsageBillingDedup(ctx, dedupCutoff)
	if dedupErr != nil {
		logger.LegacyPrintf("service.dashboard_aggregation", "[DashboardAggregation] usage_billing_dedup 保留cleanupfailed: %v", dedupErr)
	}
