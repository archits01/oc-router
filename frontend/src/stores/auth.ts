...
const TOKEN_EXPIRES_AT_KEY = 'token_expires_at' // 存储Expired时间戳而非Valid期
...
export const useAuthStore = defineStore('auth', () => {
  ...
  const tokenExpiresAt = ref<number | null>(null) // Expired时间戳（毫秒）
  ...
  /**
   * Directly set token (used for OAuth/SSO callback) and load current user info.
   * Automatically reads refresh_token and token_expires_in already set in localStorage.
