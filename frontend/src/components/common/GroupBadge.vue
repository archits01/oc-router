...
interface Props {
  ...
  userRateMultiplier?: number | null // User专属倍率
  showRate?: boolean
  daysRemaining?: number | null // 剩余days数（订阅类型时使用）
  /**
   * 订阅分组默认在右侧 label 展示"订阅"或剩余days数；
   * 开启后订阅分组也改为显示倍率（保留订阅主题色 label，配合Available Channels这类
   * 只关心费率、不关心Valid期的场景）。
   */
