 ...
 * User侧「Available Channels」聚合查询：渠道 + User可访问的分组 + 支持模型（含定价）。
 */
...
export interface UserAvailableGroup {
  ...
  /** 分组默认倍率。User专属倍率（若有）通过 /groups/rates 获取后在前端 join。 */
...
/**
 * 渠道下单个平台的子视图：User可访问的分组 + 该平台Supported Models。
...
/** 列出当前User可见的「Available Channels」（与 /groups/available 保持一致，Back平数组）。 */
